...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        $sql1 = "INSERT INTO entradas(horaEntrada, id_Codigo) VALUES (?,?)";
        ...
        $stmt->bind_param("si", $horaEntrada, $id_Codigo);
        ...
        // Verificar si ya existe una entrada registrada para el código QR
        $sqlCheck = "SELECT COUNT(*) AS count FROM entradas WHERE id_Codigo = ?";
        $stmtCheck = $conn->prepare($sqlCheck);
        if ($stmtCheck === false) {
            throw new Exception("Error al preparar la consulta de verificación: " . $conn->error);
        }
        $stmtCheck->bind_param("i", $id_Codigo);
        $stmtCheck->execute();
        ...
        if ($rowCheck['count'] > 0) {
            // Ya existe una entrada registrada, actualizar la caducidad
            $sqlUpdate = "UPDATE codigoqr 
            SET caducidad = (SELECT horaFin FROM junta 
                WHERE codigoqr.horaInicio=junta.horaInicio 
                AND codigoqr.sala=junta.sala 
                AND codigoqr.id_Anfitrion=junta.id_Anfitrion 
                AND codigoqr.fecha = junta.fecha)
            WHERE id_Codigo = ?";
            ...
            $stmtUpdate->bind_param("i", $id_Codigo);
        ...
        $conn->commit();     
        echo json_encode(array("success" => true, "message" => "El registro de la entrada y la actualización de la caducidad se han realizado correctamente."));
    } catch (Exception $e) {
